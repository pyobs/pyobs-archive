...
AuthenticationConfig.ready()) rather than needing an interactive `createsuperuser` step on a
fresh deployment. ADMIN_PASSWORD_HASH is already a Django-format hash (make_password() output,
per settings.py's own instructions) - assigned straight to User.password rather than re-hashed.
"""
